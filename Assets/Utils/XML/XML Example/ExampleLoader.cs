...
public class ExampleLoader : MonoBehaviour
{
    ...
    void Update()
    {
        if (finishedLoading)
        {
            SceneManager.LoadScene("XMLTest"); //Only happens if coroutine is finished
    ...
    void LoadXML()
    {
        ...
        xmlDoc = XDocument.Load("Assets/UI/InGame/XML/circles_test.xml");
    ...
    IEnumerator AssignData()
    {
        ...
        {
            /*Determine if the <page number> attribute in the XML is equal to whatever our current
             iteration of the loop is. If it is, then we want to assign our variables to the value 
             of the XML Element that we need.*/
            if (item.Parent.Attribute("number").Value == (iteration + 1).ToString())
            {
                ...
                /*Create a new Index in the List, which will be a new XMLData object and pass
                 the previously assigned variables as arguments so they get assigned to the new 
                 object’s variables.*/
                data.Add(new XMLData(pageNum, charText, dialogueText));

                /*To test and make sure the data has been applied to properly, print out the musicClip
                 name from the data list’s current index. This will let us know if the objects in the 
                 list have been created successfully and if their variables have been assigned the 
                 right values.*/
